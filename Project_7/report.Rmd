...
* Net conversion: The net conversion is the evaluation metric representing the number of students who keep subscribed after the free trial period.
    + The hypothesis is that it doesn't decrease significantly for the experiment group. For that reason, it is an evaluation metric.
...
* Retention: Retention is the number of user-ids enrolled after the trial period divided by the user-ids completing the checkout.
    ...
    + However, I will not use retention as an evaluation metric in this experiment because it does not add more value to our experiment. It equals the probability of a payment divided by the probability of the enrollment.  
...
According to the numbers given above, there are 40000 unique visitors per day. To have 685.275 pageviews, the experiment will have to run for ``r round(685275/40000,0)`` days.
...
The experiment is not ethically problematic and the data is not sensitive. It might be possible to draw some conclusions from the information. For example, if users are able to spend 30 hours on the course, it's unlikely that they have a full-time job. But this information is not very confidential.

There is a risk that net conversion will decrease significantly, which would mean a loss of money for Udacity.
...
#### Hypothesis
Sending users emails to invite them to get some expert advice will increase the number of enrollments after the free trial period.

#### Metrics
The metrics would be similar to the metrics used in this experiment.
